        ...
        match c {
            ...
            '/' => {
                if self.matches('/') {
                    ...
                    self.advance();
                    let mut nested_depth: u64 = 1;
                    while nested_depth > 0 && !self.is_at_end() {
                        if self.peek() == '*' && self.peek_next() == '/' && !self.is_at_end() {
                            nested_depth -= 1;
                            self.advance();
                            self.advance();
                        } else if self.peek() == '/' && self.peek_next() == '*' && !self.is_at_end()
                        {
                            nested_depth += 1;
                            self.advance();
                            self.advance();
                    ...
                    if self.is_at_end() && nested_depth > 0 {
                        self.errors.push(Error {
                            message: format!("unterminated comment block"),
                            line: self.line,
                            col: self.col,
                        });
                    }
